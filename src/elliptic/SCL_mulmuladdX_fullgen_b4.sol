...
uint256 constant _Qx = 0x00;
uint256 constant _Qy = 0x20;
uint256 constant _Qx2pow128 = 0x40;
uint256 constant _Qy2pow128 = 0x60;
uint256 constant _modp = 0x80;
uint256 constant _a = 0xa0;
uint256 constant _gx = 0xc0;
uint256 constant _gy = 0xe0;
uint256 constant _gpow2p128_x = 0x100;
uint256 constant _gpow2p128_y = 0x120;
...
function ecGenMulmuladdX_store(
    ...
    assembly ("memory-safe") {
        ...
        //allocate memory for 15 projective points, first slot is unused
        {
            ...
            X, Y, ZZ, ZZZ :=
                ecAddn2(mload(Q), mload(add(Q, 32)), 1, 1, mload(add(Q, _gx)), mload(add(Q, _gy)), _modulusp) //G+Q
            ...
            X, Y, ZZ, ZZZ :=
                ecAddn2(X, Y, ZZ, ZZZ, mload(add(Q, _gx)), mload(add(Q, _gy)), mload(add(mload(0x40), _Ap))) //Q'+G'+G
            mstore4(mload(0x40), 1408, X, Y, ZZ, ZZZ)

            X, Y, ZZ, ZZZ :=
                ecAddn2(
                    mload(Q), mload(add(Q, 32)), 1, 1, mload(add(Q, 64)), mload(add(Q, 96)), mload(add(mload(0x40), _Ap))
                ) //Q+Q'
            mstore4(mload(0x40), 1536, X, Y, ZZ, ZZZ)

            X, Y, ZZ, ZZZ :=
                ecAddn2(X, Y, ZZ, ZZZ, mload(add(Q, _gx)), mload(add(Q, _gy)), mload(add(mload(0x40), _Ap))) //Q+Q'+G
            ...
            X, Y, ZZ, ZZZ :=
                ecAddn2(X, Y, ZZ, ZZZ, mload(add(Q, 0xc0)), mload(add(Q, _gy)), mload(add(mload(0x40), _Ap))) //G'+Q+Q'+G
            //  Prec[15]
        ...
        for {} iszero(ZZZ) { mask := shr(1, mask) } {
            ZZZ :=
                add(
                    add(sub(1, iszero(and(scalar_u, mask))), shl(1, sub(1, iszero(and(shr(128, scalar_u), mask))))),
        ...
        for {} gt(mask, 0) { mask := shr(1, mask) } {
            ...
            mstore(
                add(Mem, _y2),
                addmod(mulmod(mload(add(Mem, add(32, T1))), ZZZ, _p), mulmod(Y, mload(add(Mem, _zzz2)), _p), _p)
            ) //R=S2-S1, sub avoided
            ...
            X :=
                addmod(
                    addmod(mulmod(mload(add(Mem, _y2)), mload(add(Mem, _y2)), _p), sub(_p, T2), _p),
                    mulmod(T1, mulmod(sub(_p, 2), T4, _p), _p),
                    _p
                ) // R2-PPP-2*U1*PP
            T4 := mulmod(T1, T4, _p)
            ///Q=U1*PP
            Y :=
                addmod(
                    mulmod(addmod(T4, sub(_p, X), _p), mload(add(Mem, _y2)), _p),
                    mulmod(mulmod(Y, mload(add(Mem, _zzz2)), _p), T2, _p),
                    _p
                ) // R*(Q-X3)-S1*PPP
        } //endloop
        /* IV. Normalization */
