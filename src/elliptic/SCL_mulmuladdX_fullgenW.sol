...
uint256 constant __Qx = 0x00;
uint256 constant __Qy = 0x20;
uint256 constant __modp = 0x40;
uint256 constant __a = 0x60;
uint256 constant __gx = 0x80;
uint256 constant __gy = 0xa0;
...
function ecGenMulmuladdB4W(
    ...
    assembly ("memory-safe") {
            ...
            ZZZ :=
                add(
                    add(
                        ...
                        shl(1, sub(1, iszero(and(scalar_u, mask))))
                    ), //p1
                    add(
                        shl(2, sub(1, iszero(and(scalar_v, shr(1, mask))))), //q0
                        shl(3, sub(1, iszero(and(scalar_v, mask))))
                    ) //q1
                )
        }
        ...
        for {} gt(mask, 0) { mask := shr(2, mask) } {
            ...
            mstore(
                add(Mem, __y2),
                addmod(mulmod(mload(add(Mem, add(32, T1))), ZZZ, _p), mulmod(Y, mload(add(Mem, ___zzz2)), _p), _p)
            ) //R=S2-S1, sub avoided
            ...
            X :=
                addmod(
                    addmod(mulmod(mload(add(Mem, __y2)), mload(add(Mem, __y2)), _p), sub(_p, T2), _p),
            ...
            Y :=
                addmod(
                    mulmod(addmod(T4, sub(_p, X), _p), mload(add(Mem, __y2)), _p),
                    mulmod(mulmod(Y, mload(add(Mem, ___zzz2)), _p), T2, _p),
                    _p
                ) // R*(Q-X3)-S1*PPP
        } //endloop
