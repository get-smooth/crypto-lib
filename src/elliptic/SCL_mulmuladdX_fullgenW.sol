...
uint constant _modp=0x40;
uint constant _a=0x60;
uint constant _gx=0x80;
uint constant _gy=0xa0;
       ...
       // bytes memory Mem = new bytes(16*4*32);
        assembly ("memory-safe") {
         ...
         function ecAddn2(x1, y1, zz1, zzz1, x2, y2, _p) -> _x, _y, _zz, _zzz {
          ...
          X:=mload(add(Q,_gx))
          Y:= mload(add(Q,_gy))
          X, Y, ZZ, ZZZ:=ecAddn2(X,Y, 1, 1,  mload(add(Q,_Qx)), mload(add(Q,_Qy)),_modulusp)
          ...
          X:=mload(add(Q,_Qx))
          Y:= mload(add(Q,_Qy))
          X, Y, ZZ, ZZZ:=ecDbl(X,Y, 1, 1, _modulusp, mload(add(Q,0xa0)))//2Q
          ...
          X, Y, ZZ, ZZZ:=ecAddn2(X,Y,ZZ,ZZZ,  mload(add(Q,_gx)), mload(add(Q,_gy)),_modulusp)
          mstore4(mload(0x40), 1792, X, Y, ZZ, ZZZ)  //3Q+2G  
          X, Y, ZZ, ZZZ:=ecAddn2(X,Y,ZZ,ZZZ,  mload(add(Q,_gx)), mload(add(Q,_gy)),_modulusp)
        ...
        /*III. Main loop */
            ...
            //TODO, replace mul by shifts
                ...
                let T3 := mulmod(X, T2, _p) // S = X1*V
                T1 := mulmod(T1, T2, _p) // W=UV
                let T4:=mulmod(mload(add(Q,_a)),mulmod(ZZ,ZZ,_p),_p)//aZZ1^2
