...
function ecFullGenMulmuladdX(
       /* uint256 Q0,
        uint256 Q1, //affine rep for input point Q
        uint256 Q2, 
        uint256 Q3, //affine rep for precomputations*/
        //uint256 [4] memory curve,//p, a,b,x,y,n, gpow2p128_x, gpow2p128_y, pMINUS_2
        uint256 _p,
        uint256 [4] memory Q,
        uint256 scalar_u,
        uint256 scalar_v
        ...
        /*III. Main loop */
            ...
            //TODO, replace mul by shifts
                ...
                T2:=addmod(X, sub(_p, T3),_p)
                T2 := mulmod(T4, T2, _p) //-M(S-X3)=M(X3-S)
             ...
             // let T1:=add(add(sub(1,iszero(and(scalar_u, mask))), shl(1,sub(1,iszero(and(scalar_u, T4))))),
              ...
              //inlined ec_Add
               ...
               let zzz2:= mload(add(Preco,add(96,T1)))//ZZZ2
                 ...
                 let y2 := addmod(mulmod( mload(add(Preco,add(32,T1))), ZZZ, _p), mulmod(Y,zzz2, _p), _p)//R=S2-S1, sub avoided
                 ...
                 let T2 := addmod(mulmod(T4, ZZ, _p), sub(_p, mulmod(X,T1,_p)), _p)//P=U2-U1
                        ...
                        if iszero(y2) {
                            if iszero(T2) {
                                ...
                                T2:=mulmod(pMINUS_2, y2, _p)
                                //y2:=mulmod(T4, T4, _p)
                                X := addmod(y2,T2, _p) //X3=M^2-2S
                                T2:= sub(_p, X)//S-X3

                                T2 := mulmod(T4, addmod(y2, T2, _p), _p) //M(S-X3)
                  ...
                  let T3:=mulmod(ZZ, T4, _p)
                
                  ZZ := mulmod(T3, T1 ,_p)//zz3=zz1*zz2*PP
                  T1:= mulmod(X,T1, _p)
                  T3:=mulmod(ZZZ, T2,_p)
                  ZZZ := mulmod(T3, zzz2,_p) // zzz3=zzz1*zzz2*PPP

                  T3:=mulmod(pMINUS_2, T4, _p)
                  T3:=mulmod(T1, T3, _p)//-2*U1*PP
                  X:= sub(_p, T2)//-PPP
                  T1:=mulmod(y2, y2, _p)//R^2
                  X:=addmod(X, T1, _p)//R^2-PPP
                  
                  //X:=addmod(X, T3, _p) //R2-PPP-2*U1*PP //stacks too deep here <------------------
                  //let T5:=addmod(X, T3, _p)
                  //X := addmod(addmod(mulmod(y2, y2, p), sub(p, T2), p), T3, _p)// R2-PPP-2*U1*PP

                  T4 := mulmod(T1, T4, _p)///Q=U1*PP
                  Y := addmod(mulmod(addmod(T4, sub(p, X), p), y2, p), mulmod(mulmod(Y,zzz2, p), T2, _p), _p)// R*(Q-X3)-S1*PPP
...
//second attempt: trying to solve stack to deep using local memory 
uint256 constant _zzz2=0x80;
uint256 constant _y2=0xa0;
...
function ecGenMulmuladdX_store(
       /* uint256 Q0,
        ...
        /*III. Main loop */
            ...
            //TODO, replace mul by shifts
                for {} gt(mask, 0) { mask := shr(1, mask) } {
                    let _p:=mload(add(Q, 0x80))//todo, make a first store
             ...
             // let T1:=add(add(sub(1,iszero(and(scalar_u, mask))), shl(1,sub(1,iszero(and(scalar_u, T4))))),
              ...
              //inlined ec_Add
               ...
               let T4:=mload(add(Preco,T1))//X2
               mstore(_zzz2, mload(add(Preco,add(96,T1))))//ZZZ2
                 
                
                mstore(_y2, addmod(mulmod( mload(add(Preco,add(32,T1))), ZZZ, p), mulmod(Y,mload(_zzz2), p), p))//R=S2-S1, sub avoided
                 T1:=mload(add(Preco,add(64,T1)))//zz2
                 let T2 := addmod(mulmod(T4, ZZ, p), sub(p, mulmod(X,T1,p)), p)//P=U2-U1
                        ...
                        if iszero(mload(_y2)) {
                            if iszero(T2) {
                                ...
                                mstore(_y2, mulmod(X, T2, p)) // S = X1*V
                                ...
                                X := addmod(mulmod(T4, T4, p), mulmod(pMINUS_2, mload(_y2), p), p) //X3=M^2-2S
                                T2 := mulmod(T4, addmod(mload(_y2), sub(p, X), p), p) //M(S-X3)
                  ...
                  T1:= mulmod(X,T1, _p)
                  ZZZ := mulmod(mulmod(ZZZ, T2, _p), mload(_zzz2),_p) // zzz3=zzz1*zzz2*PPP
                  X := addmod(addmod(mulmod(mload(_y2), mload(_y2), _p), sub(_p, T2), _p), mulmod( T1 ,mulmod(pMINUS_2, T4, _p),_p ), _p)// R2-PPP-2*U1*PP
                  T4 := mulmod(T1, T4, _p)///Q=U1*PP
                  Y := addmod(mulmod(addmod(T4, sub(_p, X), _p), mload(_y2), _p), mulmod(mulmod(Y,mload(_zzz2), _p), T2, _p), _p)// R*(Q-X3)-S1*PPP
