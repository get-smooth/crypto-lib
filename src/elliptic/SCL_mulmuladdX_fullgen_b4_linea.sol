 ...
 * 0 is mapped to 0 (cause ZZ=0 ->X=O in the only use made for this internal)
 */
    function _invmod(uint value, uint p) view
         returns (uint)
    {
       if(value==0){
        return 0;
...
function ecGenMulmuladdX_linea(
             ...
             // let T1:=add(add(sub(1,iszero(and(scalar_u, mask))), shl(1,sub(1,iszero(and(scalar_u, T4))))),
              ...
              //inlined ec_Add
               ...
               if iszero(ZZ) {
                ...
                scalar_u:=mload(add(mload(0x40), _Ap))//load p
          }//end assembly
          scalar_v=_invmod(ZZ, scalar_u);// 1/ZZ mod p
          X = mulmod(X, scalar_v, scalar_u);
